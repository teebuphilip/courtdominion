"""
Main automation pipeline orchestrator - WITH REAL NBA DATA.
...
from ingest_injuries import ingest_injuries              # ✅ CORRECT
from projection_generator import generate_projections    # ✅ CORRECT
...
class AutomationPipeline:
    """
    Main automation pipeline with REAL NBA DATA.
    ...
    def __init__(self, data_dir: Path = None):
        ...
        self.logger.section("COURTDOMINION AUTOMATION PIPELINE - REAL NBA DATA")
        self.logger.info(f"Data directory: {self.data_dir}")
        self.logger.info(f"Started at: {datetime.utcnow().isoformat()}")
    
    def run(self) -> bool:
        """
        Run complete automation pipeline with REAL DATA.
        ...
        try:
            ...
            if write_success:
                self.logger.section("PIPELINE COMPLETED SUCCESSFULLY - REAL NBA DATA")
    ...
    def _write_manifest(self, success: bool, error: str = None):
        ...
        """
        manifest = {
            "pipeline_version": "1.0.0-real-data",
            "data_source": "NBA.com + ESPN + DBB2",
            "success": success,
...
def run_pipeline(data_dir: Path = None) -> bool:
    """
    Main entry point for automation pipeline - REAL NBA DATA.
