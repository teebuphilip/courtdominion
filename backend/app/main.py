...
def run_projections(req: ProjReq):
    # find legacy runner script
    entry = None
    for root, dirs, files in os.walk(os.path.join(os.getcwd(), "legacy")):
        if "dbb2_main.py" in files:
            entry = os.path.join(root, "dbb2_main.py")
            break
    if not entry:
        raise HTTPException(status_code=500, detail="Legacy runner dbb2_main.py not found")
